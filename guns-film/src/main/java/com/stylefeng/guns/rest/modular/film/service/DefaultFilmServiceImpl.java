...
import com.stylefeng.api.film.FilmServiceApi;
...
public class DefaultFilmServiceImpl implements FilmServiceApi {
    ...
    private List<FilmInfo> getFilmInfos(List<MoocFilmT> moocFilms) {
        ...
        for (MoocFilmT moocFilmT : moocFilms) {
            ...
            filmInfo.setScope(moocFilmT.getFilmScore());
            filmInfo.setImgAddress(moocFilmT.getImgAddress());
            filmInfo.setFilmType(moocFilmT.getFilmType());
            filmInfo.setFilmScore(moocFilmT.getFilmScore());
            filmInfo.setFileName(moocFilmT.getFilmName());
    ...
    public FilmVO getHotFilms(boolean isLimit, int nums, int nowPage, int sortId, int sourceId, int yearId, int catId) {
        ...
        if (isLimit) {
            ...
            // 如果sorceId，yearId,catId不为99， 则表示按照对应的编号进行查询
            if (sourceId != 99) {
    ...
    public List<CatVO> getCats() {
        ...
        // 将实体对象转换为业务对象
        for (MoocCatDictT moocCatDitT : moocCats) {
            CatVO catVO = new CatVO();
            catVO.setCatId(moocCatDitT.getUuid() + "");
            catVO.setCatName(moocCatDitT.getShowName());
